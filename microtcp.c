                  ...
                  socklen_t address_len)
{
  ...
  if(PRINTS) printf("--CLIENT-- INIT_WIN = %d CURR_WIN = %d\n", socket->init_win_size, socket->curr_win_size);
}
               ...
               int flags)
{
  ...
  socklen_t address_len = socket->address_len;
  microtcp_header_t client, server;
  if(PRINTS) printf("data sent: %d, length: %d\n", data_sent, length);
  ...
  while(data_sent < length){
    ...
    if(PRINTS) printf("Received %d | REMAINING = %d CWND = %d SSTHRESH = %d WINDOWSIZE = %d\n", to_send, remaining, socket->cwnd, socket->ssthresh, socket->curr_win_size);
    ...
    for(i = 0; i < chunks; i++){
      ...
      if(PRINTS) printf("Sending chunk of 1400 number %d\n", i);
