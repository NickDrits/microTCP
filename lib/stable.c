 ...
 */
/*recvfrom is the receive function from the socket lib we are using our rec in the code if it doesn't
 *use the recvfrom */
#include "microtcp.h"
#include <stdlib.h>
#include <time.h>
...
microtcp_socket (int domain, int type, int protocol)
{
  ...
  microtcp_sock = socket(domain, SOCK_DGRAM, protocol);//(AF_INET, SOCK_DGRAM, 0)
  if (microtcp_sock == -1)
	{
  ...
  // Set timeout find the struct create it
               ...
               socklen_t address_len)
{
  ...
  /* Makes sure the sin field is cleared */
  //memset(&sin, 0, sizeof(sin));
  //sin.sin_family = AF_INET;
  //sin.sin_port = htons(1024); // listening_port =1024 || equal 0 to let the system choose the port
  //sin.sin_addr.s_addr = htol (INADDR_ANY); /* all network interfaces */
  //sin.sin_addr.s_addr = inet_addr("192.168.1.1"); specific interface
                 ...
                 socklen_t address_len)
{
  ...
  //if (microtcp_bind == -1){
  //  if(DEBUG) printf("3-Way handshake failed. Port number hasn't been assgned, connection couldn't be established\n");
  //  return -1 ; 
  //}
  ...
  if(ntohl(client.ack_number) == ntohl(server.seq_number) + 1){
    if(ntohs(client.control) == ACK){
      if(DEBUG) printf("3-Way handshake completed\n");
      socket->seq_number = ntohl(client.ack_number);//M+1
      socket->state = ESTABLISHED;
      //return 0;
    }else{
      socket->state = INVALID;
      if(DEBUG) printf("3-Way handshake failed, connection couldn't be established\n");
      //return -1;
    }
  }else{
    socket->state = INVALID;
