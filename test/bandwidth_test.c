...
int received = -1;
  ...
  /* Opens file in writing mode to copy the data from the network */
  fyle = fopen (file, "w");
  if (!fyle) {
...
/* Create UDP socket */
  ...
  // Socket keep track
  sock.address = sin; // Keep track of address
  sock.address_len = sizeof(struct sockaddr_in); // Keep track of address length
  ...
  while((received = microtcp_recv(&sock, (void*)buffer, CHUNK_SIZE, 0)) > 0){
    fwrite(buffer, sizeof(uint8_t), received, fyle);
    memset(buffer, '\0', 3 * CHUNK_SIZE);
  }

  clock_gettime (CLOCK_MONOTONIC_RAW, &end_time);
  print_statistics (sock.bytes_received, start_time, end_time);
...
//ueloume to buffer na exei etoimo size?
  /* Start sending the data */
  printf ("Sending data...\n");
  while (!feof (fyle)) {
    read_items = fread (buffer, sizeof(uint8_t), CHUNK_SIZE, fyle);
  ...
  //sleep(1); 
  //printf ("Data sent. Shutting down...\n");
